...
export async function PUT(
  ...
  try {
    ...
    if (!ObjectId.isValid(id)) {
      return NextResponse.json({ success: false, error: 'Invalid checklist ID' }, { status: 400 });
    }

    const body = await request.json();
    const validation = qcChecklistSchema.partial().safeParse(body);
    ...
    const result = await db.collection(COLLECTIONS.QC_CHECKLISTS).findOneAndUpdate(
      { _id: new ObjectId(id) },
      { $set: data },
    ...
    return NextResponse.json({ success: true, data: result, message: 'Checklist updated' });
  } catch (error) {
    console.error('QC checklist PUT error:', error);
...
export async function DELETE(
  ...
  try {
    ...
    console.error('QC checklist DELETE error:', error);
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 });
  }
}
